...
export function AccountInfo({
  ...
  const movementColor = rankChange24h > 0 ? theme.colors.primary : theme.colors.error;
  const movementSign = rankChange24h > 0 ? "+" : "";
  ...
  return (
    <View style={{ marginVertical: 12, alignItems: "center" }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "baseline" }}>
        <Text style={{ fontSize: 48, fontWeight: "bold" }}>{rank}</Text>
        <Text style={{ fontSize: 20, fontWeight: "bold", color: movementColor, marginLeft: 8 }}>
          ({movementSign}{rankChange24h})
        </Text>
      </View>
      <View>
        ...
        <Text className="text-sm">
          {winningTrades} W / {losingTrades} L ({winningTrades + losingTrades} trades)
        </Text>
      </View>
...
export function AccountActions({ onShare }: { onShare: () => void }) {
  return (
    <View style={{ marginVertical: 12, alignItems: "center" }}>
      <Button mode="contained" onPress={onShare} icon="share-variant">
        Share on X
      </Button>
    </View>
  );
